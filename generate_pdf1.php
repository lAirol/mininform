        ...
        if ($isJur) {
            ...
            if (!$isState) {
                ...
                if (is_array($nested) && count($nested) > 0) {
                    $pdf->WriteWrapped(5, ind($level + 1) . 'Учредители указанного юридического лица:');
                    renderFounders($pdf, $nested, $p, $level + 1, $childrenKey);
                }
            }
        } else {
            $pdf->SetFont('DejaVu', '', 10);
    ...
    foreach ($items as $i => $o) {
        ...
        if ($isJur) {
            ...
            if (!$isState) {
                ...
                if (is_array($nested) && count($nested) > 0) {
                    $pdf->WriteWrapped(5, ind($level + 1) . 'УЧРЕДИТЕЛИ УКАЗАННОГО ЮРИДИЧЕСКОГО ЛИЦА:');
                    renderOwners($pdf, $nested, $p, $level + 1, $childrenKey);
                }
            }
        } else {
