...
class SmiJsonValidator {
    public function validate(array $data): array {
        $errors = [];
        $add = function (string $msg) use (&$errors): void {
            $errors[] = $msg;
        };
        ...
        if (!is_array($main)) {
            ...
            if ($this->isEmpty($this->get($main, 'name'))) $add('mainInfo.name: поле обязательно');
            if ($this->isEmpty($this->get($main, 'typeSmi'))) $add('mainInfo.typeSmi: поле обязательно');
            if ($this->isEmpty($this->get($main, 'kindSmi'))) $add('mainInfo.kindSmi: поле обязательно');

            if ($this->trim($this->get($main, 'kindSmi') ?? '') === 'Другое' && $this->isEmpty($this->get($main, 'kindSmiOther'))) {
            ...
            if (!empty($main['subjectsOtherEnabled']) && $this->isEmpty($this->get($main, 'subjectsOther'))) {
                $add('mainInfo.subjectsOther: заполните при включённой доп. тематике');
            }

            if ($this->isEmpty($this->get($main, 'distributionTerritoryDetailExtra'))) {
                $add('mainInfo.distributionTerritoryDetailExtra: поле обязательно');
            }

            if (($this->trim($main['periodicity'] ?? '') === 'Другое') && $this->isEmpty($this->get($main, 'periodicityOther'))) {
                $add('mainInfo.periodicityOther: заполните при выборе «Другое»');
            }
            if (($this->trim($main['volumeVoice'] ?? '') === 'Другое') && $this->isEmpty($this->get($main, 'volumeVoiceOther'))) {
                $add('mainInfo.volumeVoiceOther: заполните при выборе «Другое»');
            }
            if (($this->trim($main['distributionTerritory'] ?? '') === 'Другое') && $this->isEmpty($this->get($main, 'distributionTerritoryDetail'))) {
                $add('mainInfo.distributionTerritoryDetail: заполните при выборе «Другое»');
            }

            $langOther = $main['languageOther'] ?? null;
            if ($langOther !== null && $langOther !== '' && $langOther !== [] && $this->isEmpty($this->get($main, 'languageOtherText'))) {
                $add('mainInfo.languageOtherText: заполните при выборе «Другое» по языкам');
            }

            $bynVol = $main['bynVolume'] ?? null;
            if ($bynVol !== null && $bynVol !== '' && !$this->validPercent($bynVol, 100)) {
        ...
        $hasDomainOwner = is_array($domainOwner) && (
            $this->trim($domainOwner['name'] ?? '') !== '' ||
            $this->trim($domainOwner['domainName'] ?? '') !== ''
        );
        ...
        if (!$hasFounders && !$hasDomainOwner) {
            ...
            $d = $domainOwner;
            if ($this->isEmpty($this->get($d, 'name'))) $add('domainOwner.name: поле обязательно');
            if ($this->isEmpty($this->get($d, 'country'))) $add('domainOwner.country: поле обязательно');
            if ($this->isEmpty($this->get($d, 'address'))) $add('domainOwner.address: поле обязательно');
            ...
            if ($typeFace === 'jurFace') {
                ...
                } elseif (!$this->validPercent($cap, 20)) {
            ...
            foreach ($merged as $i => $f) {
                ...
                $typeFace = $this->trim($f['typeFace'] ?? '');
                $isJur = $typeFace === 'jurFace' || array_key_exists('capitalPercent', $f) || array_key_exists('isState', $f);

                if ($isJur) {
                    if ($this->isEmpty($this->get($f, 'name'))) $add("founders[{$i}].name: поле обязательно");
                    if ($this->isEmpty($this->get($f, 'country'))) $add("founders[{$i}].country: поле обязательно");
                    if ($this->isEmpty($this->get($f, 'address'))) $add("founders[{$i}].address: поле обязательно");
                    ...
                    if (!$this->isStateOrg($f)) {
                        ...
                        elseif (!$this->validPercent($cap, 20)) $add("founders[{$i}].capitalPercent: диапазон от 0 до 20");
                    }

                    if (!$this->isStateOrg($f)) {
                        $nestErr = $this->validateNestedFoundersSum($f['founders'] ?? [], "founders[{$i}].founders");
                        foreach ($nestErr as $e) $add($e);
                    }
                } else {
                    if ($this->isEmpty($this->get($f, 'fullName'))) $add("founders[{$i}].fullName: поле обязательно");
                    if ($this->isEmpty($this->get($f, 'citizenship'))) $add("founders[{$i}].citizenship: поле обязательно");
                    if ($this->isEmpty($this->get($f, 'address'))) $add("founders[{$i}].address: поле обязательно");
                    if ($this->isEmpty($this->get($f, 'phoneCode'))) $add("founders[{$i}].phoneCode: поле обязательно");
                    if ($this->isEmpty($this->get($f, 'phone'))) $add("founders[{$i}].phone: поле обязательно");
                    if ($this->isEmpty($this->get($f, 'email'))) $add("founders[{$i}].email: поле обязательно");
        ...
        if (!is_array($office)) {
            ...
            if ($this->isEmpty($this->get($office, 'fullName'))) $add('office.fullName: поле обязательно');
            if ($this->isEmpty($this->get($office, 'country'))) $add('office.country: поле обязательно');
            if ($this->isEmpty($this->get($office, 'address'))) $add('office.address: поле обязательно');
            if ($this->isEmpty($this->get($office, 'phoneCode'))) $add('office.phoneCode: поле обязательно');
            ...
            if (!is_array($owners) || count($owners) === 0) {
                ...
                $ownerErr = $this->validateOwnersBlock($owners, 'office.owners');
                foreach ($ownerErr as $e) $add($e);
            }

            $chief = $office['chiefEditor'] ?? [];
            if (is_array($chief)) {
                if ($this->isEmpty($this->get($chief, 'name'))) $add('office.chiefEditor.name: поле обязательно');
                if ($this->isEmpty($this->get($chief, 'country'))) $add('office.chiefEditor.country: поле обязательно');
                if ($this->isEmpty($this->get($chief, 'actAssignment'))) $add('office.chiefEditor.actAssignment: поле обязательно');
                if ($this->isEmpty($this->get($chief, 'institution'))) $add('office.chiefEditor.institution: поле обязательно');
                if ($this->isEmpty($this->get($chief, 'formerJobInfo'))) $add('office.chiefEditor.formerJobInfo: поле обязательно');
    ...
    private function get(array $arr, string $key) {
        return $arr[$key] ?? null;
    }
    ...
    private function isStateOrg($it): bool {
        if (!is_array($it)) return false;
        $v = $it['isState'] ?? null;
        return $v === true || $v === 'true';
    }

    private function validateNestedFoundersSum(array $items, string $path): array {
        $err = [];
        ...
        foreach ($items as $i => $it) {
            if (!is_array($it) || $this->isStateOrg($it)) continue;
            $nested = $it['founders'] ?? [];
            if (is_array($nested) && count($nested) > 0) {
                $sub = $this->validateNestedFoundersSum($nested, "{$path}[{$i}].founders");
    ...
    private function validateOwnersBlock(array $items, string $path): array {
        $err = [];
        if (count($items) === 0) return $err;
        $total = 0;
        foreach ($items as $it) {
        ...
        foreach ($items as $i => $it) {
            ...
            $nested = $it['owners'] ?? [];
            if (is_array($nested) && count($nested) > 0) {
                $sub = $this->validateOwnersBlock($nested, "{$path}[{$i}].owners");
                foreach ($sub as $e) $err[] = $e;
            }
        }
        return $err;
...
function validateSmiJson(array $data): array {
    return (new SmiJsonValidator())->validate($data);
}
